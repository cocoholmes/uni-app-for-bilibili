		...
		<view class="contentContainer">
			...
			<view class="leftContainer">
				<scroll-view scroll-y="true" class="navScroll">
					<view class="navItem" :class="{active: navIndex === index}" @click="changeNavIndex(index)" v-for="(item, index) in realCategory" :key='item.id'>
						{{item.name}}
					</view>
				</scroll-view>
			</view>
...
<script>
	...
	export default {
		...
		mounted() {
			this.getRealCategory()
		},
		methods: {
			async getRealCategory(){
				this.realCategory = await request('/getRealCategory')
			},
			// 点击导航的回调
		...
		computed: {
			cateObj(){
				return this.realCategory[this.navIndex]
			}
		}
	}
</script>
